...
docker run --rm --dns "$build_dns" \
  ...
  node:22-alpine \
  sh -c 'mkdir /tmp/collector && cp -R /src/package.json /src/package-lock.json /src/tsconfig.json /src/src /tmp/collector/ && cd /tmp/collector && npm ci && npm test && npm prune --omit=dev && cp package.json package-lock.json /out/ && cp -R node_modules dist /out/'
