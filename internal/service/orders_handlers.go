...
<script>let updating=false;async function update(){if(updating||document.hidden)return;updating=true;try{const response=await fetch(location.pathname,{cache:'no-store',headers:{Accept:'text/html'}});if(!response.ok)return;const documentCopy=new DOMParser().parseFromString(await response.text(),'text/html');const next=documentCopy.getElementById('live'),current=document.getElementById('live');if(next&&current&&next.dataset.version!==current.dataset.version){const open=!!current.querySelector('details[open]');current.replaceWith(next);if(open)next.querySelector('details')?.setAttribute('open','')}}catch(error){}finally{updating=false}}setInterval(update,1000);document.addEventListener('visibilitychange',()=>{if(!document.hidden)update()});</script>
</body></html>`
