...
<div class="actions"><form method="post" action="/order-auction-flipper/watch"><input type="hidden" name="signature" value="{{.Signature}}"><button type="submit">Recheck now</button></form><code>{{.OrderCommand}}</code><span>then</span><code>{{.AuctionCommand}}</code><span class="muted">order observed {{clock .OrderFreshAt}}</span></div></article>
