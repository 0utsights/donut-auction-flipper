...
<div class="numbers"><div class="number"><span class="muted">Market capacity</span><strong>{{moneyCents .OrderUnitRewardCents}} each</strong><span class="muted">up to {{.MaxOrderQuantity}} units / {{.ExecutableBatches}} exit listings; Fabric sizes this to your balance</span></div><div class="number"><span class="muted">Each auction exit</span><strong>{{.Quantity}}× list for {{money .TargetListPrice}}</strong><span class="muted">{{money .ExpectedProceeds}} conservative proceeds after fee · reuse the 18 slots</span></div><div class="number profit"><span class="muted">Profit per exit listing</span><strong>+{{money .ConservativeProfit}}</strong><span class="muted">{{pct .MarginBPS}} ROI</span></div><div class="number"><span class="muted">Total market opportunity</span><strong>{{money .PriorityScore}} / day</strong><span class="muted">one order per item · {{.AuctionVolume24h}} sales{{if .Profiled}} · proven profile{{end}}</span></div></div>
