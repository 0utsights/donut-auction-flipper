...
<header><div><h1>Order → Auction Flips</h1><div class="muted">Only verified markets are shown. Fabric performs a focused recheck before creation.</div></div><a href="/order-auction-flipper/debug">Debug</a></header>
<div id="live" data-version="{{.Version}}"><div class="status"><strong>{{.ReadyCount}} ready</strong> · updated {{clock .GeneratedAt}} · live</div>
<main id="good-orders">
{{range .Ready}}<article class="flip"><div class="top"><div><span class="rank">#{{.PriorityRank}}</span> <span class="name">{{.Quantity}}× {{.ItemName}}</span><div class="muted"><code>{{.ItemID}}</code></div></div><div><strong>READY FOR RECHECK</strong></div></div>
<div class="numbers"><div class="number"><span class="muted">Market capacity</span><strong>{{moneyCents .OrderUnitRewardCents}} each</strong><span class="muted">up to {{.MaxOrderQuantity}} units / {{.ExecutableBatches}} exit listings; Fabric sizes this to your balance</span></div><div class="number"><span class="muted">Each auction exit</span><strong>{{.Quantity}}× for {{money .TargetListPrice}}</strong><span class="muted">{{money .ExpectedProceeds}} after fee · reuse the 18 slots</span></div><div class="number profit"><span class="muted">Profit per exit listing</span><strong>+{{money .ConservativeProfit}}</strong><span class="muted">{{pct .MarginBPS}} ROI</span></div><div class="number"><span class="muted">Total market opportunity</span><strong>{{money .PriorityScore}} / day</strong><span class="muted">one order per item · {{.AuctionVolume24h}} sales{{if .Profiled}} · proven profile{{end}}</span></div></div>
