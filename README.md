...
Copy the remapped JAR from `minecraft/client-mod/build/libs/` to the instance's `mods` directory. The verified build is checked in at `outputs/donut-market-flips-2.1.0-alpha.21.jar`. Press `N` or run `/dn`.
...
The `/dn` screen is a compact blue/black order console: live balance, deployable cash, slot use, portfolio economics, and the four current orders are visible without tooltips. Adjustment and diagnostic controls live under `Local State` instead of competing with the portfolio. Every unavailable action displays its exact blocker; the automatic-consent action remains clickable when blocked so it can explain the missing balance, candidate, connection, or slot state. `Review` retains one-order consent. `Auto Orders` opens a second confirmation screen that can authorize the currently allocated queue for the current Minecraft session. Fabric processes one item at a time and performs a new focused recheck. Before opening the server menu, it may adopt updated economics only for the identical canonical item and only within the consent-time escrow cap; otherwise that untouched item is skipped after a bounded wait. After navigation begins, any uncertainty stops the whole queue. Fabric verifies every 1.21.11 screen and economic value, then proves exactly one matching item exists in `Your Orders` before continuing. Submitted items are persisted as active across restarts and excluded from allocation. Claiming filled orders and auction relisting are not implemented yet.
