...
Legacy inferred fill reductions are retained rather than erased, but have confirmation level zero and are excluded from scoring. A fill counts only when the same observer sees the same stable order key at the same price decrease within two minutes, on the same menu page and Minecraft connection session. Page/session matching prevents two distinct orders occupying the same slot on adjacent pages from masquerading as a reduction. This quarantines impossible long-gap and cross-page volume while new focused watches rebuild trustworthy fill velocity. The dashboard exposes confirmed and quarantined totals separately.
