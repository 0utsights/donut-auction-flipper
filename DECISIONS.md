...
Legacy inferred fill reductions are retained rather than erased, but have confirmation level zero and are excluded from scoring. A fill counts only when the same observer sees the same stable order key and price decrease within two minutes, on the same menu page of the same leased `focused_watch` task, and the reduced row's canonical signature exactly matches that task's target signature. Discovery scans and neighboring items on a watched page can never create trusted fill volume. Page/task/signature matching prevents two distinct orders occupying the same slot on adjacent pages, or an unrelated busy item beside the target, from masquerading as target volume while allowing the collector's deliberately idempotent refresh-session IDs to rotate. This quarantines impossible long-gap, cross-page, and neighbor-item volume while focused watches rebuild trustworthy fill velocity. The dashboard exposes confirmed and quarantined totals separately.
