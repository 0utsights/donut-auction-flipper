...
Donut expires the order GUI after roughly 80 seconds even when its verified refresh control is used. A focused-watch menu expiry is therefore retried by reopening `/orders` on the same authenticated Minecraft connection; it is not treated as a network failure and does not force a login/proxy reconnect. Discovery completion still rotates the connection to avoid reusing stale navigation state.
...
Order discovery continues to scan every verified global page because the server menu's ordering and search control have not been proven safe or stable enough to assume that page one contains every desirable item. When a refreshed candidate feed first exposes a viable target—or after a complete discovery pass—the backend schedules one bounded two-minute focused sample. It chooses among economically viable order-to-auction candidates using the official auction API's exact-quantity resale target, preferring `READY` over `RESEARCH`, then higher resale value, then the existing risk-adjusted profit score. Automatic samples have a global one-minute minimum interval and rotate on a five-minute per-item cooldown. A player-requested watch remains higher priority and retains its longer four-minute research window. This creates a high-value lane without starving broad discovery or clicking an unverified search UI.
...
- DonutSMP observer and execution automation require explicit written server-team authorization. On 2026-08-26 the published DonutSMP terms prohibited macros, scripts, auto-clickers, inventory modifications, and modifications that provide unfair advantages. The live Mineflayer collector was therefore stopped, and its manager now requires `DN_SERVER_OBSERVER_AUTHORIZED=true` before launching any account. The default is false, including after a container restart. Fabric may display backend information and prepared values, but it must not navigate dialogs, create/claim orders, purchase, or relist automatically without that authorization. The official terms checked were `https://store.donutsmp.net/terms/checkout`.
