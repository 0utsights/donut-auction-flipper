...
Fabric rechecks quote freshness at the purchase-selection boundary, not only when entering the workflow. Shulker spend is capped cumulatively per position at the smaller of the conservative remaining profit and the balance-scaled aggregate cap. Once received, an empty or filled package is accepted from player inventory only when its sole changed component is its container; custom names, lore, custom data, enchantments, colored variants, and all other component changes stop the session.

Immediately before the verified market row is clicked, Fabric durably records `SUPPLY_PENDING` because Donut's Quick Buy mode may make that first click transactional. It restores the previous progress state only after the exact empty-box inventory delta is visible. A restart or persistence failure across that boundary cannot replay the purchase: the pending state fails preflight and requires manual reconciliation.

Fabric's exact server allowlist includes `play.donutsmp.net`, the apex `donutsmp.net`, and the current official SRV target `java.donutsmp.net`. The resolved target was added after live diagnostics proved Minecraft was connected and authenticated while the mod rejected the host as unapproved. The allowlist remains explicit rather than accepting arbitrary subdomains.

The backend maintains this supply with a dedicated official-API query (`shulker_box`, `lowest_price`) every five seconds. It does not reuse the broad auction scan because a full rate-limited traversal can leave an otherwise-live listing cached for hours. The server timestamps only a successfully completed targeted observation and returns HTTP 503 after 20 seconds without one; it never refreshes an old row's envelope timestamp. Fabric independently enforces the same row-age limit and therefore waits safely through API or collector outages.
