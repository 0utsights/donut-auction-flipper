...
Legacy inferred fill reductions are retained rather than erased, but have confirmation level zero and are excluded from scoring. A fill counts only when the same observer sees the same stable order key and price decrease within two minutes, on the same menu page of the same leased `focused_watch` task. Discovery scans can never create trusted fill volume. Page/task matching prevents two distinct orders occupying the same slot on adjacent pages from masquerading as a reduction, while allowing the collector's deliberately idempotent refresh-session IDs to rotate. This quarantines impossible long-gap and cross-page volume while focused watches rebuild trustworthy fill velocity. The dashboard exposes confirmed and quarantined totals separately.
