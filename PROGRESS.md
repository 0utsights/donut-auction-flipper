...
- Live collector and transaction-automation work is paused pending explicit DonutSMP staff authorization. The published rules prohibit macros/scripts; the collector manager now defaults to launching zero accounts unless `DN_SERVER_OBSERVER_AUTHORIZED=true` is deliberately set.
