...
- The distinct `2.1.0-alpha.3` Fabric client polls the combined candidate feed, parses or lets the player override balance, applies a dynamic reserve, and uses a deterministic Pareto optimizer to allocate bulk acquisition orders within 20 order and 18 auction slots. It permits only one active order per item, persists local duplicate locks, verifies the server's Your Orders screen before every creation, and stops on direct duplicate-order server messages. Its Fabric-only executor preserves cent-precise stack economics, verifies each chest/dialog transition, repeats freshness/budget/slot checks immediately before `Create Order`, and stops on any ambiguity. DonutSMP staff authorization for observer collection and locally armed Fabric order creation was explicitly confirmed by the operator on 2026-08-26.
